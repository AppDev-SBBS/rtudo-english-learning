...
const createUserIfNotExists = async (user) => {
  ...
  if (!snap.exists()) {
    // First-time signup — add welcome bonus
    const onboarding = {
    ...
    // Award daily XP if not already awarded today
    if (lastLoginXpDate !== today) {
      const dailyXP = 10;

      // Ensure source object exists
      if (!todayXP.source || typeof todayXP.source !== 'object') {
        todayXP.source = {};
      }
      ...
      // Update timestamp even if no XP added
      await setDoc(userRef, {
        lastLoginDate: nowISO,
        updatedAt: nowISO,
      }, { merge: true });
