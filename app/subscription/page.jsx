...
export default function SubscriptionPlans() {
  ...
  const fetchUserPlans = async () => {
    ...
    if (subSnap.exists()) {
      ...
      setUserPlan(data?.current?.plan || null);
  ...
  <div className="rounded-3xl border overflow-hidden mb-6 shadow-sm transition-all hover:shadow-md opacity-100">
    ...
    >
      ...
      <p className="text-lg font-bold">
        INR {plan.price} <span className="text-base font-medium">/month</span>
      </p>
      {plan.popular && (
  ...
  const InvoiceCard = ({ plan }) => (
    <div className="border rounded-xl p-4 my-4 bg-gray-50 shadow-sm">
      <h3 className="text-lg font-bold text-[var(--color-primary)] mb-2">📄 Invoice</h3>
      <p><strong>Plan:</strong> {plan.plan.toUpperCase()}</p>
      <p><strong>Amount:</strong> ₹{plan.amount}</p>
      <p><strong>Start Date:</strong> {new Date(plan.startDate.seconds * 1000).toLocaleDateString()}</p>
      <p><strong>End Date:</strong> {new Date(plan.endDate.seconds * 1000).toLocaleDateString()}</p>
      <p><strong>Status:</strong> {plan.status}</p>
      <p><strong>Order ID:</strong> {plan.razorpay_order_id}</p>
    </div>
  );
  ...
  {userPlan?.plan === "pro" && (
    <>
      <InvoiceCard plan={userPlan} />
      <PlanCard plan={{ ...plans.basic, disabled: true }} />
    </>
  )}
</div>
